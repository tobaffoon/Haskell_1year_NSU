...
primend x | prime x = primend' 2 x (div x 2) + 1
          | otherwise = primend' 2 x (div x 2)
                where primend' n x cmp_const | n > cmp_const = 0
                                             | divides x n && prime n = 1 + primend' (n+1) x cmp_const
                                             | otherwise = primend' (n+1) x cmp_const